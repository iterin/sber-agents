...
Предоставляет инструменты для банковского агента:
...
async def calculate_deposit_profit(
    ...
    - Полученный доход
    - Эффективную годовую ставку с учетом срока и капитализации
    """
    logger.info(
        "calculate_deposit_profit called: "
        f"initial_amount={initial_amount}, annual_rate={annual_rate}, "
        f"term_months={term_months}, capitalization={capitalization}"
    )
    ...
    if capitalization:
        # Месячная капитализация процентов
        monthly_rate = rate / 12.0
        final_amount = principal * (1 + monthly_rate) ** months
    else:
        # Простой процент без капитализации
        final_amount = principal * (1 + rate * (months / 12.0))

    profit = final_amount - principal

    # Эффективная годовая ставка (APY), пересчитанная с учетом фактического срока
    # и способа начисления процентов
    if months > 0 and principal > 0:
        total_factor = final_amount / principal
        effective_annual_rate = (total_factor ** (12.0 / months) - 1) * 100.0
    else:
    ...
    result = (
        ...
        "Исходные параметры:\n"
        f"- Сумма вклада: {principal:,.2f} RUB\n"
        f"- Ставка: {annual_rate:.2f}% годовых\n"
        f"- Срок: {months} месяцев\n"
        f"- Капитализация: {'да' if capitalization else 'нет'}\n\n"
        "Результаты:\n"
        f"- Итоговая сумма: {final_amount:,.2f} RUB\n"
        f"- Доход: {profit:,.2f} RUB\n"
        f"- Эффективная ставка: {effective_annual_rate:.2f}% годовых"
    )
