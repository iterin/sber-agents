...
def format_chunks(chunks):
    ...
    if not chunks:
        logger.warning("No chunks to format - returning default message")
    ...
    for i, chunk in enumerate(chunks, 1):
        ...
        # Отладка: логируем вопрос из метаданных
        logger.info(f"Chunk {i}: question='{question}' source='{source_name}'")

    result = "\n\n---\n\n".join(formatted_parts)
    logger.info(f"Formatted result length: {len(result)}")
    return result
...
def get_retrieval_query_transformation_chain():
    ...
    # Обернем в функцию для отладки
    def debug_chain(input_data):
        result = chain.invoke(input_data)
        logger.info(f"Query transformation result: '{result}'")
        return result

    return debug_chain

def get_rag_chain():
    """Финальная RAG-цепочка - используем оригинальный вопрос для поиска"""
    if retriever is None:
        raise ValueError("Retriever not initialized")

    conversational_answering_prompt, _ = _load_prompts()

    # Функция для извлечения последнего вопроса пользователя и выполнения поиска
...
async def rag_answer(messages):
    ...
    # Отладка: получим последний пользовательский вопрос
    ...
    logger.info(f"Processing question: '{last_user_message}'")
