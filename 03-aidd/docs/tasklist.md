...
| 3 | Память диалога (≤20) | ⏳ | 80% | Код готов, тест |
| 4 | Ошибки/тайм-ауты | ⏳ | 80% | Код готов, тест |
...
- [x] Память переписки, обрезка до ≤20 последних сообщений
- [x] Очистка контекста по `/reset`
- [ ] Тест: отправить >20 сообщений и убедиться, что ранние обрезаются

### Итерация 4 — Обработка ошибок
- [x] Тайм-ауты и повтор запроса (ограниченно)
- [x] Пользовательские фолбэки при ошибках (вежливые сообщения)
- [ ] Тест: смоделировать тайм-аут/ошибку и проверить фолбэк

### Итерация 5 — Конфигурация и запуск
- [x] ENV-переменные (токены, модель, тайм-ауты)
- [x] Makefile/скрипт для локального запуска
- [x] Тест: запуск через одну команду с разными ENV

### Итерация 6 — Логирование (минимум)
- [x] Структурные логи: вход, выход, ошибки (уровни)
- [x] Корреляция по user/chat (id)
- [x] Тест: проверить записи логов для типовых сценариев

### Итерация 7 — Базовые команды UX
- [x] `/start` кратко объясняет возможности
- [м] `/help` показывает команды и подсказки
- [ ] Тест: проверить ответы `/start` и `/help`

### Итерация 8 — Рейт-лимит
- [x] Лимит сообщений на пользователя за окно времени
- [x] Вежливое уведомление при превышении
- [x] Тест: вызвать лимит, получить уведомление
